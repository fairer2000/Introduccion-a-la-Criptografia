...
def InvsubByte(s):
  for i in range(4):
    for j in range(4):
      s[i][j] = Invsbox[int(s[i][j][0],16)][int(s[i][j][1],16)]
...
def product(num, opt):
    if opt == 9:
        return int(Mul_9[int(num[0],16)][int(num[1],16)],16)
    elif opt == 11:
        return int(Mul_11[int(num[0],16)][int(num[1],16)],16)
    elif opt == 13:
        return int(Mul_13[int(num[0],16)][int(num[1],16)],16)
    elif opt == 14:
        return int(Mul_14[int(num[0],16)][int(num[1],16)],16)
...
def InvmixColumns(s):
    ...
    for i in range(4):
        for j in range(4):
            aux = product(s[0][i],Invmtx[j][0]) ^ product(s[1][i],Invmtx[j][1]) ^ product(s[2][i],Invmtx[j][2]) ^ product(s[3][i],Invmtx[j][3])
            result[j].append(format(aux, '02x'))
    return result


def decrypt(msg,dw,Nb,Nr,Nk):
    k=0
    state = [[],[],[],[]]
    ...
    for i in range(4): #Pasa el caracter del arreglo (o string) a una matriz.
        for j in range(4):
            ...
            k=k+1
    state = addRoundKey(state,dw[Nr*Nb:(Nr+1)*Nb])
  
    for rnd in range(Nr-1,0,-1):
        state = InvshiftRows(state)
        state = InvsubByte(state)
        state = addRoundKey(state,dw[rnd*Nb:(rnd+1)*Nb])
        state = InvmixColumns(state)
    state = InvshiftRows(state)
    state = InvsubByte(state)
    state = addRoundKey(state,dw[0:Nb])
  
    return state

def decrypt_hex(msg,dw,Nb,Nr,Nk):
    k=0
    state = [[],[],[],[]]
    array_hex = [msg[i:i+2] for i in range(0, len(msg), 2)]
    for i in range(4): #Pasa el caracter del arreglo (o string) a una matriz.
        for j in range(4):
            state[j].append(array_hex[k])  #
            k=k+1
    print(state)
    state = addRoundKey(state,dw[Nr*Nb:(Nr+1)*Nb])
...
def AES(mes,k,Nk,Nb,Nr,option):
    w = keyExpansion(k,Nb,Nr,Nk)
    if option == 1:
        return decrypt(mes,w,Nb,Nr,Nk)
    elif option == 2:
        return decrypt_hex(mes,w,Nb,Nr,Nk)
  
if __name__ == "__main__":
    opt = int(input("Ingresar texto plano o texto cifrado: "))    
    message = input("Mensaje a descifrar: ")
    key = input("Llave: ")
    if(len(key) == 16):
        nk = 4
        nr = 10
        ...
        nr = 14
        nb = 4
    #AES(message,key,nk,nb,nr,option)
    save = AES(message,key,nk,nb,nr,opt)
